	...
	public List<String> tabComplete(CommandSender sender, String alias, String[] args) throws IllegalArgumentException {
		...
		if(args.length == 1) {
			...
			for(DynamicArgument<?> argument : rawArguments) {
				collectInputs(sender, args, 0,0, result, argument);
			}
		}else {
	...
	private List<String> argumentNames(CommandSender sender, String[] args, DynamicArgument argument, int depth) {
		...
		for(DynamicArgument<?> dynamicArgument : argument.subArguments()) {
			if(depth != args.length - 1) {
				if(dynamicArgument.isValid(join(Arrays.copyOfRange(args, depth, depth + dynamicArgument.span())))) {
					if(dynamicArgument.span() > 1) {
						for(int i = 0; i < dynamicArgument.span(); i++) {
							if(depth+i != args.length-1)continue;
							collectInputs(sender, args, depth+i, i, result, dynamicArgument);
						}
			...
			} else collectInputs(sender, args, depth, 0, result, dynamicArgument);
		}
		return result;
	}

	private void collectInputs(CommandSender sender, String[] args, int index, int offset, List<String> result, DynamicArgument<?> dynamicArgument) {
		...
		if(suggestions != null) {
			for(String suggestion : suggestions) {
				String[] suggests = suggestion.split(" ");
				boolean matches = true;
				if(suggests.length > 1)
					for(int i = offset-1; i > 0; i--) {
						if(suggests.length-i < 0) break;
						if(!suggests[suggests.length-1-i].equalsIgnoreCase(args[index - i])) {
							matches = false;
							break;
						}
					}
				if(matches && suggests[Math.min(suggests.length-1, offset)].toLowerCase().startsWith(args[index].toLowerCase())) {
					result.add(join(Arrays.copyOfRange(suggests, offset, suggests.length)));
				}
			}
		}
	}
