...
public abstract class DynamicSubCommand<C extends DynamicCommand> {
  ...
  public DynamicArgument<?>[] rawArguments() {return rawArguments;}

  public Map<String, String> arguments() {return arguments;}

  public String[] permissions() {return new String[]{command.owner().getName().toLowerCase() + ".command." + command.getName() + "." + name()};}


  public List<String> aliases(){return new ArrayList<>();}
  ...
  public boolean canUse(String[] args) {return arguments.isEmpty() || arguments.keySet().stream()
    .anyMatch(key-> compare(args,key.split(" ")));}

  public boolean canUse(DynamicArguments args) {return arguments.isEmpty() || arguments.keySet().stream()
