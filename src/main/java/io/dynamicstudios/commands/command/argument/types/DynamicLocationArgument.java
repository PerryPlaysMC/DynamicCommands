...
public class DynamicLocationArgument extends DynamicArgument<Location> {
 ...
 @Override
 public Location parse(CommandSender sender, String input) throws CommandException {
	if(sender instanceof Player) {
	 ...
	 if(in[0].startsWith("^"))
		position.setX(location.getX() + location.getDirection().multiply(Double.parseDouble(in[0].substring(1).isEmpty() ? "0" : in[0].substring(1))).getX());
	 if(in[1].startsWith("^"))
		position.setY(location.getY() + location.getDirection().multiply(Double.parseDouble(in[1].substring(1).isEmpty() ? "0" : in[1].substring(1))).getY());
	 if(in[2].startsWith("^"))
		position.setZ(location.getZ() + location.getDirection().multiply(Double.parseDouble(in[2].substring(1).isEmpty() ? "0" : in[2].substring(1))).getZ());

	 if(!in[0].equals("~") && !in[0].startsWith("^"))
		position.setX(position.getX() + Double.parseDouble(in[0].startsWith("~") ? in[0].substring(1) : in[0]));
	 if(!in[1].equals("~") && !in[1].startsWith("^"))
		position.setY(position.getY() + Double.parseDouble(in[1].startsWith("~") ? in[1].substring(1) : in[1]));
	 if(!in[2].equals("~") && !in[2].startsWith("^"))
		position.setZ(position.getZ() + Double.parseDouble(in[2].startsWith("~") ? in[2].substring(1) : in[2]));
