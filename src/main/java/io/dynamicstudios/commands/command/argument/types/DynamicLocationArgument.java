...
public class DynamicLocationArgument extends DynamicArgument<Location> {
  ...
  public boolean isValid(String input) {
      ...
      boolean isValid = inputs.length == 3 && Arrays.stream(inputs).allMatch(s->s.isEmpty()||s.matches("~|~?-?\\d+(\\.\\d+)?"));
      if(!isValid) {
        int count = 0;
        for(char c : input.toCharArray()) {
          if(c == ' ') count++;
        }
        isValid = count == 2 && input.replace(" ", "").matches("(~|~?-?\\d+(\\.\\d+)?)+");
      }
      return isValid;
  }
  ...
  public Location parse(CommandSender sender) throws CommandException {
    if(sender instanceof Player) {
      ...
      if(in[0].startsWith("~")) position.setX(((Player)sender).getLocation().getX());
      if(in[1].startsWith("~")) position.setY(((Player)sender).getLocation().getY());
      if(in[2].startsWith("~")) position.setZ(((Player)sender).getLocation().getZ());

      if(!in[0].equals("~")) position.setX(position.getX() + Double.parseDouble(in[0].startsWith("~") ? in[0].substring(1) : in[0]));
      if(!in[1].equals("~")) position.setY(position.getY() + Double.parseDouble(in[1].startsWith("~") ? in[1].substring(1) : in[1]));
      if(!in[2].equals("~")) position.setZ(position.getZ() + Double.parseDouble(in[2].startsWith("~") ? in[2].substring(1) : in[2]));
